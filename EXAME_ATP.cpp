...
int jogar_ate_o_desempate(){
  int jog1, jog2, resultado;
  do{
    jog1 = rand() % 3;
    jog2 = rand() % 3;
    resultado = primeira_jogada(jog1, jog2);
  }while(resultado == 0);
  return resultado;   
}
...
void confronto(Membros* m1, Membros* m2){
  ...
  m1->movimento_favorito = jog1;
  m2->movimento_favorito = jog2;
}
  ...
  for(int i = 0; i <NUM_MEMBROS; i++){
    for(int j = i + 1; j < NUM_MEMBROS; j++){
      ...
      if(resultado == 1){
        ...
        membros[i]->movimento_favorito = jog1;
        membros[j]->movimento_favorito = jog2;
      }
    }
  
  fprintf(k, "Matriz de Resultados (0 = empate, 1 = linha venceu, 2 = coluna venceu)\n\n");
  for(int i = 0; i < NUM_MEMBROS; i++){
...
Membros* eliminatorias_final(const char* nome_arquivo){
  ...
  fprintf(k, "=== QUARTAS DE FINAL ===\n\n");
  
  Membros* quartas[4];
  fprintf(k, "Partida 1: %s vs %s\n", membros[0]->nome, membros[7]->nome);
  quartas[0] = jogar_ate_o_desempate() == 1 ? membros[0] : membros[7];
  fprintf(k, "Vencedor: %s\n\n", quartas[0]->nome);
  
  fprintf(k, "Partida 2: %s vs %s\n", membros[1]->nome, membros[6]->nome);
  quartas[1] = jogar_ate_o_desempate() == 1 ? membros[1] : membros[6];
  fprintf(k, "Vencedor: %s\n\n", quartas[1]->nome);
  
  fprintf(k, "Partida 3: %s vs %s\n", membros[2]->nome, membros[5]->nome);
  quartas[2] = jogar_ate_o_desempate() == 1 ? membros[2] : membros[5];
  fprintf(k, "Vencedor: %s\n\n", quartas[2]->nome);
  
  fprintf(k, "Partida 4: %s vs %s\n", membros[3]->nome, membros[4]->nome);
  quartas[3] = jogar_ate_o_desempate() == 1 ? membros[3] : membros[4];
  fprintf(k, "Vencedor: %s\n\n", quartas[3]->nome);
  

  fprintf(k, "=== SEMIFINAIS ===\n\n");
  Membros* semis[2];
  fprintf(k, "Semifinal 1: %s vs %s\n", quartas[0]->nome, quartas[3]->nome);
  semis[0] = jogar_ate_o_desempate() == 1 ? quartas[0] : quartas[3];
  fprintf(k, "Vencedor: %s\n\n", semis[0]->nome);

  fprintf(k, "Semifinal 2: %s vs %s\n", quartas[1]->nome, quartas[2]->nome);
  semis[1] = jogar_ate_o_desempate() == 1 ? quartas[1] : quartas[2];
  fprintf(k, "Vencedor: %s\n\n", semis[1]->nome);
  
  fprintf(k, "=== FINAL ===\n\n");
  fprintf(k, "Final: %s vs %s\n", semis[0]->nome, semis[1]->nome);
...
int main(){
  ...
  Membros* campeao = eliminatorias_final("eliminatorias.txt");
